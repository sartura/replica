...
if [ -n "${DTB}" ]; then
	FDT_NODE="
		fdt-1 {
			description = \"${ARCH_UPPER} Replica ${DEVICE} device tree blob\";
			data = /incbin/(\"${DTB}\");
			type = \"flat_dt\";
			arch = \"${ARCH}\";
			compression = \"none\";
...
/ {
	description = \"${ARCH_UPPER} Replica FIT (Flattened Image Tree)\";
	#address-cells = <1>;

	images {
		kernel-1 {
			description = \"${ARCH_UPPER} Replica Linux-${VERSION}\";
			data = /incbin/(\"${KERNEL}\");
			type = \"kernel\";
			arch = \"${ARCH}\";
			os = \"linux\";
